...
class Transport:
    ...
    def sample(self, x1, sp_timesteps=None, shifted_mu=0):
        ...
        if not self.use_lognorm:
            ...
            if not self.shift_lg:
                if self.partitial_train is not None and th.rand(1) < self.partial_ratio:
                    ...
                    t = self.sample_in_range(self.lognorm_mu, self.lognorm_sigma, x1.shape[0], range_min=self.partitial_train[0], range_max=self.partitial_train[1])
                else:
        ...
        if self.train_timestep_shift != 1.0:
            s = self.train_timestep_shift
            t = (s * t) / (1 + (s - 1) * t)
